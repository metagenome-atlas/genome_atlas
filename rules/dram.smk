...
def get_all_genome_names():
    ...
    return glob_wildcards(f"{genome_dir}/{{genome}}/{config['fasta_extension']}").genome
...
rule DRAM_annotate:
    ...
    output:
        outdir=temp(directory("annotations/dram/intermediate_files/{genome}")),
    threads: config["threads"]
    resources:
...
rule concat_annotations:
    input:
        expand(rules.DRAM_annotate.output.outdir, genome=get_all_genome_names())
    output:
